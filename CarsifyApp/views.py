...
def Dashboard(request):
    ...
    paginator = Paginator(car_data, 9)
    ...
    dic = {'car_data': car_data, 'car_company_data': car_company_data, \
           ...
           'car_body_types':Body_Types, 'Minmaxprice':minmaxPrice, 'minmaxYear': minmaxYear, \
               'minmaxKm': minmaxKm, 'states':states,'fav_id':fav_ids, 'states':states }
...
def filter_data(request):
    ...
    car_state = request.GET.getlist('state[]')
    ...
    if len(car_state) > 0:
        car_data = car_data.filter(Registration_State__id__in=car_state).distinct()
...
# Image -> save -> resize -> save resized -> delete original
def resize_img_show(img_path, img_name):
    pic=cv2.imread(img_path)
...
def Delete_From_Favourite(request,cid):
    ...
    if request.method == "GET":
        t = render_to_string('allcarlist.html', {'car_data': car_data,'fav_id':fav_ids})
        return JsonResponse({'data':t})
...
def GetImages(request,id):
    data = Individual_Car_Details.objects.filter(id=id)[0]
    images = Individual_Car_Images.objects.filter(carid=data)
    print(images.values())
    t = render_to_string('dashboardcaraousel.html', {'data': images})
    return JsonResponse({'data':t})
...
def MailOTP(request):
    if request.method == 'POST':
        ...
        subject, from_email, to = '[Carsify] Verify Your Email', 'carsify.in@gmail.com',email
        text_content = 'Please use the following OTP : '+ str(otp) + ' to verify your email'
        html_content = '<p>Please use the following OTP : <strong>'+ str(otp) + '</strong> to verify your email</p>'
        msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
        msg.attach_alternative(html_content, "text/html")
        ...
        global OTP
        OTP = otp
        return JsonResponse({'data':otp,'exist':0})
...
# Dealer's End Views
def DealerApplicationView(request):
    if request.POST:
        x = request.POST
        full_name = x.get('full_name', None)
        email = x.get('email', None)
        contact  = x.get('contact', None)
        doc = request.FILES.get('doc')
        password = x.get('password', None)
        currPassword = request.user.password
        matchcheck = check_password(password,currPassword)
        print(currPassword)
        print(password)
        print(matchcheck)
        if matchcheck:
            d = DealerApplication.objects.create(user = request.user, Name=full_name, Email=email, Contact=contact, Document=doc)  
            subject, from_email, to = '[Carsify] New Dealer Application', 'carsify.in@gmail.com', 'vaibhavgupta942466@gmail.com'
            text_content = "New Dealer Application has been registered. Kindly visit the admin dashboard and verify the dealer. \nDealer's details:\nName:  "+d.Name+"\nEmail: "+d.Email+"\nContact: "+d.Contact+"\n"
            html_content = '<p>New Dealer Application has been registered. Kindly visit the admin dashboard and verify the dealer.</p> <p>Dealer\'s details:</p> <p>Name:  '+d.Name+'</p> <p>Email: '+d.Email+'</p> <p>Contact: '+d.Contact+'</p>'
            msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
            msg.attach_alternative(html_content, "text/html")
            msg.send()
            return redirect("CarsifyApp:Dashboard")
        else:
            return JsonResponse({'error':1,'msg':'Incorrect Password'})
            
        return JsonResponse({'data':0})
    return render(request, 'registration.html')
        # JSONResponse has been submitted for AJAX Queries. If using post method, then use:
        # But here need to pass the dictionary for form submission and show a successful application
        # return render(request, 'dealerapplication.html')
...
def registerDealer(request):
    if request.POST:
        ...
        Company_Name = x.get('Company_Name', None)
        Contact  = x.get('contact', None)
        Email = x.get('email', None)
        doc = request.FILES.get('doc')
        d = DealerApplication.objects.create(user = request.user, Name=Name, Email=Email, Contact=Contact, Document=doc, location=location, Company_Name=Company_Name)
        return JsonResponse({'data':1})
...
def DealerDashboard(request):
    ...
    cities = []
    for i in Individual_Car_Details.objects.filter(Car_Status=False).values():
    ...
    return render(request, 'dealerdashboard.html', {'dealer':dealer,'profile':profile,'city':city,'recommendations':recommendations})
